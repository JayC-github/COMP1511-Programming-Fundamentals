...
int stDate_to_num(struct date *start_day);
int fiDate_to_num(struct date *finish_day);
int Date_to_num(struct pod *first_pod);
...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
    ...
    while (pointer != NULL) {
        if (Date_to_num(pointer) >= stDate_to_num(start_day) && Date_to_num(pointer) <= fiDate_to_num(finish_day)) {
            printf("%02d/%02d/%02d %2d %s\n", pointer->when->date, pointer->when->month, pointer->when->year, pointer->how_many, pointer->species);
...
int stDate_to_num(struct date *start_day) {
    int num;
    num = (start_day->year)*365 + (start_day->month)*30+(start_day->day);
    return num;
}

int fiDate_to_num(struct date *finish_day) {
    int num;
    num = (finiish_day->year)*365 + (finish_day->month)*30+(finish_day->day);
    return num;
}

int Date_to_num(struct pod *first_pod) {
    int num;
    num = (first_pod->when->year)*365 + (first_pod->when->month)*30 + (first_pod->when->day);
    return num;
}
//
