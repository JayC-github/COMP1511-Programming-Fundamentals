...
//
// This program by Jay (z5261536) on 2019.03.28
...
int count_arkles(int tiles1[N_TILES], int tiles2[N_TILES]) {
    int arkles = 0;
    int Usedtiles1[N_TILES] = {0};// [0,0,0,0]
    int Usedtiles2[N_TILES] = {0};// [0,0,0,0] 
//make the farnarkles postion become 1
    int i = 0;
    while(i < N_TILES) {
        if (tiles1[i] == tiles2[i]) {
            Usedtiles1[i] = 1;
    ...
    int k = 0;//Tiles1-1,2,3,4
        while (k < N_TILES) {
            int l = 0;//1,1 1,2 1,3 1,4 2,1 2,2 2,3 2,4...
            while (l < N_TILES) {
                if(Usedtiles1[k]== 0 && Usedtiles2[l] == 0 ){
                    if (tiles1[k] == tiles2[l]){
                        Usedtiles1[k] = 1;
                        Usedtiles2[l] = 1;
                        arkles++;
                    }
...
void play_farnarkle(int hidden_tiles[N_TILES]) {
    int i = 0;
    int j = 1; //round number
    int tiles[N_TILES];
    while(i == 0) {
        printf("Enter guess for turn %d: ", j);
        int k = 0;
        while (k < N_TILES){
            scanf("%d", &tiles[k]);
            k++;
        ...
        int fark = count_farnarkles(tiles, hidden_tiles);
        int ark = count_arkles(tiles,hidden_tiles);
